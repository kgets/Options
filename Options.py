...
def requestOptChain(ticker,isCalls):
    ...
    #filter out zero bids and wide spreads
    returnDf = returnDf[(returnDf.bid>0) & (returnDf.ask-returnDf.bid < 1)]    
    #add annualized interest column
    estComm = 1.50 #Hard coded rough estimated commision & Fees cost for trade.
    returnDf['AnnualizedInterest'] = 0
    #ITM calc
    returnDf.AnnualizedInterest = returnDf.AnnualizedInterest.where(price < returnDf.strike, round((((price-returnDf.logMidPrice-estComm/100)/returnDf.strike)**(52/(returnDf.DTE//7+1))*100),2))
    #OTM calc
    returnDf.AnnualizedInterest = returnDf.AnnualizedInterest.where(price >= returnDf.strike,   round((((returnDf.logMidPrice-estComm/100)/price+1)**(52/(returnDf.DTE//7+1))*100),2))
...
ticker = 'slv'
optChain = requestOptChain(ticker,True)
